...
if (!empty($_FILES) && $_POST['token'] == $verifyToken) {
	...
	if (in_array($fileParts['extension'],$fileTypes)) {
		...
		$list_org_chart = mysqli_query( $sporteqa, $query_list_org_chart) or die(mysqli_error($sporteqa));
		...
		if ($totalRows_list_org_chart>0){
			...
			mysqli_select_db( $sporteqa, $database_sporteqa);
			$Result1 = mysqli_query( $sporteqa, $deleteSQL) or die(mysqli_error($sporteqa));
		}
