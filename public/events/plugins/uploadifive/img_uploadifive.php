...
$targetFolder = '/userfiles/user/'.$_POST['spq_event_id'].'/image';
...
class Image {
            ...
            function flush($spq_event_id) {
				$targetFolder = '/userfiles/user/'.$spq_event_id.'/image';
...
if (!empty($_FILES)) {
	...
	if ($fileData) {
		...
		if (in_array(strtolower($fileParts['extension']), $fileTypes) && filesize($tempFile) > 0 && isImage($tempFile)) {
			...
			// remove files under the same ID
			mysqli_select_db($sporteqa, $database_sporteqa);
			$query_get_file = sprintf("SELECT * FROM xs_file WHERE spq_file_id = %s ", GetSQLValueString($spq_reg_id, "int")); echo $query_get_file;
			$get_file = mysqli_query($sporteqa, $query_get_file) or die(mysqli_error($sporteqa));
			$row_get_file = mysqli_fetch_assoc($get_file);
			$totalRows_get_file = mysqli_num_rows($get_file);
			
			if ($totalRows_get_file>0) {
				do {
					$file_name = $row_get_file['file_name'];
					if (file_exists($targetFolder."/".$file_name)){
						unlink($targetFolder."/".$file_name);
					}
				} while ($row_get_file = mysqli_fetch_assoc($get_file));
				
				
			}


			$delete_files_first = sprintf("DELETE FROM xs_file WHERE spq_file_id=%s",
				    GetSQLValueString($id, "int"));
			mysqli_select_db( $sporteqa, $database_sporteqa); 
			$Result0 = mysqli_query( $sporteqa, $delete_files_first) or die(mysqli_error($sporteqa));
			...
			$updateSQL = sprintf("INSERT INTO xs_file (spq_file_id, file_name, file_type, file_date, file_modby, spq_event_id, spq_event_registration_id, spq_event_user_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
				    GetSQLValueString($id, "int"),
					GetSQLValueString($fileEnc, "text"),
