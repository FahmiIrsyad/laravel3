...
require_once($basedir.'/../../../Connections/sporteqa_nologin.php');
// ---------------------------------------------------------
// you must add your details below:       
$db_server = $hostname_sporteqa; // your MySQL server - localhost will normally suffice
$db = $database_sporteqa;//""; // your MySQL database name
$mysql_username = $username_sporteqa;//"";  // your MySQL username
$mysql_password = $password_sporteqa;//"";  // your MySQL password


if (isset($_REQUEST['send_email']) &&($_REQUEST['send_email']==1)){
	
	$from_emailaddress = "nikfariz@gmail.com";
	$report_emailaddress = "nikfariz@gmail.com";//address to send reports, not the backup just details on last backups (can be same as above)
	$to_emailaddress = urldecode($_REQUEST["email"]); // your email address to send backup files to

	$email_array = explode(',', $to_emailaddress);
	
} else {
	include($base_dir."/../../../common/mailer_login_basedir.php");
	$from_emailaddress = $setfrom;
