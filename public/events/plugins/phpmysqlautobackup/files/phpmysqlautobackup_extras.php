...
function xmail ($to_emailaddress,$from_emailaddress, $subject, $content, $file_name, $backup_type, $ver)
{
 ...
 $mail_content = "Penyimpanan data berjaya".NEWLINE.NEWLINE."Sila muat-turun fail yang dilampirkan; $backup_type ".NEWLINE;	
 //return mail($to_emailaddress, $subject, $mail_content, "From: $from_emailaddress".NEWLINE."Reply-To:$from_emailaddress".NEWLINE.$add_header);
	...
	$attachment = $base_dir."/../backups/".$file_name;
	include("../PHPMailer_5.2.4/mailer_login_basedir.php");
	include("../PHPMailer_5.2.4/sendmail.php");
}
...
class record
{
      ...
      {
       $dbc = dbc::instance();
       $result = $dbc->prepare("SHOW TABLES LIKE 'phpmysqlautobackup_log' ",$options="");
      ...
      {
       $dbc = dbc::instance();
       $result = $dbc->prepare("SELECT * FROM `phpmysqlautobackup_log` ORDER BY `date` DESC ", $options = NULL);
       $rows = $dbc->executeGetRows($result);
